    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (pwServer.Start())
            {
                ...
                Shop.Init();
                Util.Log("Shop OK!");
                Thread autoSave = new Thread(() => pwServer.autoSave(pwServer));
                autoSave.IsBackground = true;
                autoSave.Start();
                var start = DateTime.Now;
                if (!ConfigData.Init())
                {
                    Util.Log("Failed to load blocks config!");
                }

                Util.Log("Blocks config successfully loaded in " + (DateTime.Now - start).TotalMilliseconds.ToString() + " milliseconds");
